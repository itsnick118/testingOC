...
{
    [TestFixture]
    public class PowerpointContextRegressionTests : UITestBase
    {
        ...
        public void SetUp()
        {
            ...
            _quickFileInfo = CreateDocument(OfficeApp.Powerpoint);
            _powerpoint.OpenDocumentFromExplorer(_quickFileInfo.FullName);
            _powerpoint.AttachToOc();
            _powerpoint.Oc.BasicSettingsPage.LogInAsStandardUser();
        }

        [Test]
        [Category(RegressionTestCategory)]
        [Description("Test case reference: TC_16378 Verify the People Tab in office App(Powerpoint)")]
        public void VerifyPeopleTabInPowerpoint()
        {
            ...
            var mattersListPage = _powerpoint.Oc.MattersListPage;
            var matterDetails = _powerpoint.Oc.MatterDetailsPage;
            var documentsListPage = _powerpoint.Oc.DocumentsListPage;
            var peopleListPage = _powerpoint.Oc.PeopleListPage;
            ...
            var emailCount = _powerpoint.Oc.GetQueuedEmailCount();
            ...
            Assert.IsTrue(addedPerson.PersonType.StartsWith("Internal"));
            ...
            var messages = _powerpoint.Oc.GetAllToastMessages();
            Assert.AreEqual(1, messages.Length);
            Assert.AreEqual(MessageonTryingToRemovePIC(personPIC.PersonName), messages[0]);
            _powerpoint.Oc.CloseAllToastMessages();
        ...
        [Test]
        [Category(RegressionTestCategory)]
        [Description("Test case reference: TC_16833 working with emails in other office apps(Powerpoint)")]
        public void VerifyEmailTabInPowerpoint()
        {
        ...
        {
            SaveScreenShotsAndLogs(_powerpoint);
            _powerpoint.Close();
            _powerpoint.Destroy();
        }
    }
